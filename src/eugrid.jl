...
struct Grid
    ...
    function Grid(n::Int)
        ...
        dil = [la.norm(inner .- outer, 1) for inner=iix(), outer=LIndices(n_inner)]
        dit = permutedims(dil, (2, 1, 3))

        dg = [la.norm(l .- t, 1) for l=LIndices(n_inner), t=TIndices(n_inner)]
        de = [la.norm(l .- t, 2) for l=LIndices(n_inner), t=TIndices(n_inner)]
...
LIndices(n_inner::Int) = zip([1:n_inner; fill(n_inner, n_inner - 1)],
                             [fill(0, n_inner); 1: n_inner - 1])
