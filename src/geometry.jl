...
function Onion(kernel::Vector{Atom}, basis::Vector{Atom}, n::Int)
    kernel = sort(kernel)
    o = Onion([kernel], [Layer(GraphDistances(), kernel)])
    atoms = setdiff!(Set(Atoms(n)), kernel)
    while true
        kernel = sort!(filter(a->!isnothing(pop!(atoms, a, nothing)),
                              map(sum, Iterators.product(kernel, basis))))
        push!(o.membranes, kernel)
        isempty(kernel) && break
    end
    @assert isempty(atoms) "incomplete basis $basis does not cover $atoms"
    o
end

Base.size(o::Onion)::NTuple{2, Int} = maximum(o.membranes[end-1]).I

Base.isempty(o::Onion) = isempty(o.layers)

isfull(o::Onion) = length(o.layers) == length(o.membranes)

function wrap!(o::Onion, ds::GraphDistances)::Onion
    push!(o.layers, Layer(ds, o.membranes[length(o.layers)+1]))
    o
end

unwrap!(o::Onion)::Layer = pop!(o.layers)

function eugrid(o::Onion)
    diags = Matrix{Union{Missing, Bool}}(missing, size(o))
    for (i, d) in flatten(l.interior for l in o.layers)
